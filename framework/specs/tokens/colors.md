...
기본 상태에서 Hover, Pressed로 갈수록 팔레트 레벨을 일정 간격으로 올린다(이 저장소 계약의 기본 방향 — 모든 시스템에 보편적인 법칙은 아니다. 전이 방향·인접 레벨 이동·터치 타깃 등 상태 전반의 규칙은 [`tokens/states.md`](./states.md) 참고).
...
> 출처: `talk/script.md` 슬라이드 17. 알파 스케일 설계법과 선택 기준은 대본에는 없으나, `framework/adapters/krds/colors.md` §9, `framework/adapters/material/colors.md` §3에서 각자 다루는 투명도 활용 방식을 일반화해 보강했다.

세밀하게 관리하고 싶다면 투명도를 5단계 정도로 나누고, 배경색별 조합까지 미리 정의한다 (예: `black/4%, 8%, 16%, 32%, 64%`, 배경이 밝을 때/어두울 때 각각).

**투명도 스케일 설계 방법**은 두 갈래로 나뉜다 — 둘 다 유효하며, 어느 쪽을 기본 메커니즘으로 쓸지는 프로젝트가 정한다.

- **팔레트 위에 얹는 부가 수단**: 팔레트 단계(§3)를 상태 표현의 기본 수단으로 쓰고, 투명도는 BG/Text/Line처럼 배경에 따라 미묘하게 톤을 낮춰야 하는 경우에만 보조로 쓴다. Disabled처럼 "색 자체가 달라져야" 하는 상태에는 투명도 대신 별도 색(예: `--muted`)을 쓴다 — 반투명은 배경이 바뀌면 대비가 같이 흔들리기 때문이다.
- **상태 색상의 기본 메커니즘**: §5의 "Hover/Pressed를 팔레트 레벨로 올리는" 방식 대신, **고정된 base 색 위에 투명도 오버레이만 얹어서** Hover/Focus/Pressed를 구분한다. 이 방식을 쓰면 팔레트 단계 자체를 늘리지 않아도 되지만, 오버레이 opacity 값(예: Hover 8%, Focus/Pressed 10~12%)을 상태별로 미리 정해두고 프로젝트 전체에서 통일해야 한다.

어느 방식이든 배경 밝기(라이트/다크)별로 같은 투명도가 다르게 보일 수 있으므로, 배경 조합별 실제 결과를 §4 대비 기준으로 검산한 뒤 값을 고정한다.
