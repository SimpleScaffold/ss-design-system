...
> 출처: `talk/script.md` 슬라이드 7 (당근 60-30-10 사례) · `framework/specs/tokens/colors.md`, `radius.md`, `spacing.md`, `layout.md`, `typography.md`, `icon.md`, `states.md`의 공통 규칙을 당근의 실제 브랜드 색에 대입한 예시. 색 토큰 형식은 [`../framework/color-tokens.md`](../framework/color-tokens.md) — `[색상]` 블록은 그 프롬프트를 그대로 포함하고 브랜드 값만 다르게 채운다.
...
[상태]
- FAB·버튼 Default→Hover→Pressed는 인접 레벨로만 이동한다(건너뛰지 않는다). 키보드 포커스는 색상 외에 outline/ring 같은 비색상 단서를 함께 준다.
...
- 같은 형식의 다른 브랜드 사례: [`toss.md`](./toss.md), [`naver.md`](./naver.md)
