...
5. `framework/adapters/<시스템명>/`가 이미 있으면 그 문서를 실제 값의 1차 자료로 지목하고 링크한다. 아직 없으면 "이 문서는 철학/출처 포인터 전용이며, 실제 토큰 값이 필요해지면 [GitHub 경로]를 1차 소스로 삼아 어댑터를 새로 만들어야 한다"고 명시한다.
