...
4. "절차" — 번호 붙은 단계. 각 단계는 반드시 framework/specs/의 실제 파일(tokens/colors.md, components/component-contract.md, validation/accessibility-checklist.md 등)을 지목해서 "그 파일의 어느 섹션을 어떻게 쓰는지" 구체적으로 연결해라. 색 토큰을 만들거나 바꾸면 `prompts/framework/color-tokens.md` 계약을 절차에 명시해라. 추상적인 절차("좋은 디자인을 만든다")를 쓰지 마라.
