...
5. hex 코드나 브랜드 고유 px 값을 넣지 마라 — 팔레트 레벨(Primary-50 등)이나 토큰 이름만 써라. 실제 값은 framework/adapters/*가 채운다.
6. 마지막 섹션은 `## 토큰화`(또는 `## 토큰화 예시`) — 이 스펙을 실제 코드에 반영할 때의 추상 토큰 형식(CSS 커스텀 프로퍼티 등)을 코드 블록으로 보여줘라.
